...
export const api = {
  ...
  meta: () => request<{ records: number; people: number; years: number }>("/api/v1/meta"),
  ...
  person: (slug: string) => request<{ person: { slug: string; displayName: string; identityMode: string }; records: InterviewRecordSummary[] }>(`/api/v1/people/${encodeURIComponent(slug)}`),
