...
export function DriftPage() {
  ...
  function driftAgain() {
    if (data) {
      const next = [...seen, data.archiveNumber].slice(-12);
      setSeen(next);
    ...
    <section className="page-shell flex min-h-[calc(100dvh-4rem)] items-center py-12 tablet:min-h-[calc(100dvh-5rem)]">
      <div className="mx-auto w-full max-w-3xl">
        <div className="text-center">
          <p className="archive-label text-blueprint">DRIFT / RANDOM ENCOUNTER</p>
          <h1 className="mt-4 font-serif text-3xl font-semibold tablet:text-5xl">一份档案漂到了这里</h1>
        </div>
        ...
        {data && !loading && (
          <article className="paper-card relative mt-10 overflow-hidden p-6 tablet:p-10">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link to={`/archives/${data.archiveNumber}`} className="button-primary flex-1">打开这份档案 <ArrowRight size={16} /></Link>
