import { FilePlus2, MessagesSquare, PencilLine, ShieldCheck } from "lucide-react";
...
export function StudioPage() {
  ...
  return <section className="page-shell py-14 tablet:py-20">
    <header className="grid gap-6 border-b border-line pb-10 tablet:grid-cols-12 tablet:items-end"><div className="tablet:col-span-8"><p className="record-label text-blueprint">ACCOUNT STUDIO</p><h1 className="mt-4 font-serif text-4xl font-semibold tablet:text-6xl">你好，{me.data?.displayName}</h1><p className="mt-4 text-sm text-ink-muted">{me.data?.email} · {me.data?.role==="director"?"馆长":"成员"}</p></div><div className="flex flex-wrap gap-3 tablet:col-span-4 tablet:justify-end"><Link to="/studio/claims" className="button-secondary"><MessagesSquare size={16}/>认领申请</Link><Link to="/studio/new" className="button-primary"><FilePlus2 size={16}/>录入采访记录</Link></div></header>
