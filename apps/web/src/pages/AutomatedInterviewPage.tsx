...
export function AutomatedInterviewPage() {
    ...
    return <section className="page-shell py-14 tablet:py-20">
      <div className="mx-auto grid max-w-5xl gap-10 tablet:grid-cols-[minmax(0,1fr)_minmax(280px,0.7fr)] tablet:items-start">
        ...
        <div className="paper-card space-y-6 p-5 tablet:p-7">
          ...
          <label className="flex min-h-12 cursor-pointer items-start gap-3 border-t border-line pt-5 text-sm leading-6"><input type="checkbox" className="mt-1 size-4 accent-[var(--accent-seal)]" checked={consented} onChange={(event) => setConsented(event.target.checked)}/><span>我知道正在和自动程序对话，并愿意开始；我也知道可以随时退出。继续即表示我已阅读<Link className="text-blueprint underline" to="/privacy">隐私政策</Link>与<Link className="text-blueprint underline" to="/terms">条款与条件</Link>。</span></label>
  ...
  return <section className="page-shell min-h-[calc(100dvh-4rem)] py-8 tablet:py-14">
    <div className="mx-auto max-w-3xl">
      ...
      {state.phase === "active" ? <div className="mt-8 border border-line bg-canvas p-3 shadow-paper">
