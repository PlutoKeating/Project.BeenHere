  ...
  return (
    <section className="page-shell py-14 tablet:py-20">
      <header className="reading-column text-center">
        <p className="record-label text-blueprint">TOPIC / {data.topic.slug}</p>
      ...
      <div className="mt-14 grid gap-4 tablet:grid-cols-2 lg:grid-cols-3">{data.records.map((record) => <RecordCard key={record.id} record={record} />)}</div>
    </section>
  );
}
