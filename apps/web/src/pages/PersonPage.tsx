...
export function PersonPage() {
  ...
  if (error || !data) return <div className="page-shell py-20"><ErrorState message={error ?? "没有找到这位被采访者。"} /></div>;
  return (
    <section className="page-shell py-14 tablet:py-20">
      ...
      <div className="grid gap-4 pt-10 tablet:grid-cols-2 lg:grid-cols-3">{data.records.map((record) => <RecordCard key={record.id} record={record} />)}</div>
    </section>
  );
}
