  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api.correction({
        recordNumber: String(form.get("recordNumber") ?? "") || undefined,
    ...
    <section className="page-shell py-14 tablet:py-20">
      <div className="grid gap-12 tablet:grid-cols-12">
        <header className="tablet:col-span-5">
          <p className="record-label text-blueprint">CORRECTION DESK</p>
          <h1 className="mt-4 font-serif text-4xl font-semibold leading-tight tablet:text-6xl">更正、隐私<br />与撤回</h1>
          <p className="mt-7 max-w-md text-sm leading-8 text-ink-muted">采访记录不会静默改写，但人始终可以要求更正、补充、匿名化或停止公开。隐私与撤回请求优先处理。</p>
        </header>
        <form onSubmit={submit} className="paper-card space-y-7 p-6 tablet:col-span-7 tablet:p-10">
          <div><label className="field-label" htmlFor="recordNumber">Record Number（可选）</label><input className="field font-mono" id="recordNumber" name="recordNumber" defaultValue={params.get("record") ?? ""} placeholder="BH-000001" pattern="BH-\d{6}" /></div>
          <div className="grid gap-7 sm:grid-cols-2">
            ...
            <div><label className="field-label" htmlFor="kind">请求类型</label><select className="field" id="kind" name="kind" defaultValue="fact"><option value="fact">事实更正</option><option value="identity">身份信息更正</option><option value="privacy">隐私遮蔽</option><option value="consent">授权争议</option><option value="supplement">补充材料</option><option value="topic">话题建议</option><option value="withdrawal">撤回公开</option></select></div>
          </div>
