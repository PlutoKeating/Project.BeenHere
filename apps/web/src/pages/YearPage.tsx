  ...
  return (
    <section className="page-shell py-14 tablet:py-20">
      <header className="border-b border-line pb-10">
        <p className="archive-label text-blueprint">TIME ARCHIVE</p>
      ...
      <div className="pt-10">
        ...
        {data && data.length > 0 && <div className="grid gap-4 tablet:grid-cols-2 lg:grid-cols-3">{data.map((archive) => <ArchiveCard key={archive.id} archive={archive} />)}</div>}
        {data && data.length === 0 && <EmptyState title="这一年的架子还是空的" body="空白也属于档案馆。记录会在发生后抵达。" />}
      </div>
    </section>
  );
}
