...
本地 Worker 身份测试使用 `x-local-user-email`，且仅在 `APP_ENV=development` 时接受。初始化本地 D1：
...
- Worker secrets：`ACCESS_TEAM_DOMAIN`、`ACCESS_AUD`、`SUPERADMIN_EMAILS`
- GitHub secret：`CLOUDFLARE_API_TOKEN`
- GitHub variable：`CLOUDFLARE_ACCOUNT_ID`

Cloudflare Access 保护 `/studio*`、`/director*`、`/api/account/*` 和 `/api/director/*`，允许通过配置身份提供商验证的用户进入。Worker 根据 `SUPERADMIN_EMAILS` 授予馆长角色。
