...
**入口**：从首页首屏或账户工作台进入；登录后访问 `/studio/interview`
...
7. **本人自动认领**：整理后的内容只能先保存为带 `ingestionMethod=automated_interview` 的未公开 `RecordDraft`；当前登录账户直接成为 `claimed` 类型的被采访者本人，公开仍需本人主动操作。
8. **来源事实不可改写**：采访开始时间取点击“开始采访”时的当前时间；机器消息、时间和录入标记在整理页只读，并由 Worker 对上一版快照强制校验。受访者仍可修改或删除自己的回答。
...
账户中心
  ...
  → 正常结束或主动结束
  → 整理为未公开记录
  → 在既有 RecordForm 修改/删除自己的回答并补充最少信息；机器提问与开始时间锁定
  → 主动保存未公开草稿
```
...
| `src/components/RecordForm.tsx` | 本人回答校对、身份补充、自动来源只读、提交未公开记录 | 现有 `RecordDraft` |
...
采访引擎不访问 `fetch`、D1、账户、WebSocket 或浏览器存储。第一版没有会话恢复，避免在未确认前形成隐藏持久化；代价是刷新会丢失当前回答，页面已明确告知。采访页不建立内部滚动容器，消息列表、输入区和完成卡片都处于正常文档流；消息增加时页面底部锚点调用 `scrollIntoView`，因此只有整页一个滚动上下文。
...
- 不自动判断内容是否适合公开；采访完成后的记录标题由独立的确定性 NLP 规则从全部被采访者消息派生，摘要不由自动采访引擎生成；
- 不自动保存恢复中的会话；
